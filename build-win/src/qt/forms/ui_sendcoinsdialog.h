...
public:
    ...
    void setupUi(QDialog *SendCoinsDialog)
    {
        ...
        feeAssetSelector = new QComboBox(frameFeeSelection);
        feeAssetSelector->setObjectName(QString::fromUtf8("feeAssetSelector"));

        horizontalLayoutFee13->addWidget(feeAssetSelector);
...
"\n"
"Note:  Since the fee is calculated on a per-byte basis, a fee rate of \"100 atoms per kvB\" for a transaction size of 500 virtual bytes (half of 1 kvB) would ultimately yield a fee of only 50 atoms.", nullptr));
#endif // QT_CONFIG(tooltip)
        labelCustomPerKilobyte->setText(QCoreApplication::translate("SendCoinsDialog", "/kvB", nullptr));
#if QT_CONFIG(tooltip)
        feeAssetSelector->setToolTip(QCoreApplication::translate("SendCoinsDialog", "Pay the transaction fee in this asset. Sequentia accepts fees in any asset a block producer prices \342\200\224 no asset is privileged. If it doesn't confirm, you can replace it (RBF, on by default).", nullptr));
#endif // QT_CONFIG(tooltip)
#if QT_CONFIG(tooltip)
        checkBoxMinimumFee->setToolTip(QCoreApplication::translate("SendCoinsDialog", "Paying only the minimum fee is just fine as long as there is less transaction volume than space in the blocks. But be aware that this can end up in a never confirming transaction once there is more demand for transactions than the network can process.", nullptr));
#endif // QT_CONFIG(tooltip)
        checkBoxMinimumFee->setText(QString());
#if QT_CONFIG(tooltip)
        labelCustomFeeWarning->setToolTip(QCoreApplication::translate("SendCoinsDialog", "Paying only the minimum fee is just fine as long as there is less transaction volume than space in the blocks. But be aware that this can end up in a never confirming transaction once there is more demand for transactions than the network can process.", nullptr));
#endif // QT_CONFIG(tooltip)
