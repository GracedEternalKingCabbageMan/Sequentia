...
void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-posexpracegateheight=<n>", "SEQUENTIA: block height from which the exponential-race leader time-gate is measured in seconds of score instead of whole slot intervals. The exp-race score is a rate, not a rank -- its minimum over all stakers is Exponential(1) -- so scaling it by the whole interval leaves the network silent for whole slots (~13.5% of blocks lose at least one). Changing when a leader may produce is a coordinated HARD FORK. Same convention as -posexpraceheight: 0 means the rule is NOT gated (off), a positive H enforces it from height H. Only meaningful where the exp-race election itself is active. Custom/regtest chains only (the real chains pin it in code). Only used when -con_pos is set. (default: 1)", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
