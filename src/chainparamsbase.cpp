...
void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-poscommitteesize=<n>", "SEQUENTIA: number of committee members (the first n stakers of each slot's leader schedule) that certify each block; a strict majority must countersign. 1 disables committee certification (leader-only signing). Range 1-16. Only used when -con_pos is set. (default: 1)", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
