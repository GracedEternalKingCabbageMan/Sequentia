...
Requirements:
  ...
  * The node must be running -chain=test, fully synced to its Bitcoin anchor
    (`getanchorstatus` -> anchorstatus "ok"), and at block height 0 (a fresh
    chain). Wipe ~/.elements/testnet3 and restart if you already produced blocks.
...
def main():
    ...
    if anchor.get("anchorstatus") != "ok":
        raise CliError("Bitcoin anchor not ready (getanchorstatus=%r). Wait for "
                       "warmup, then retry." % anchor.get("anchorstatus"))
